...
日期：2026-07-07
模块：新增 `debugtools-startup-init`（可选模块，依赖 `debugtools-startup`）
状态：设计草案，待确认后写实现计划
...
目标是新增一个独立模块 `debugtools-startup-init`，提供一套通用初始化编排 API。接入方只描述初始化任务、依赖和执行方式；模块负责调度、并发、错误处理，并自动对接 `AppStartupMonitor`。
...
- 自动上报 `begin/success/fail` 到 `AppStartupMonitor`。
- 在流程结束时自动调用 `AppStartupMonitor.complete()`。
...
```text
debugtools-startup-init -> debugtools-startup
```

它不依赖 `debugtools-core`，因为它不提供 UI，只把数据送进现有启动链路模块。
...
class Builder {
    ...
    fun reportToStartupMonitor(): Builder
...
- 校验失败会通过 reporter 写入启动链路中的失败 step，便于 UI 里看到“启动编排配置错误”。
...
`SKIPPED` 不改变 `StartupStep` 的协议三态。对启动链路的记录方式是：
...
- 说明什么时候使用 `debugtools-startup-init`。
